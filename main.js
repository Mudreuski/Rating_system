...
let localObj = JSON.parse(localStorage.getItem("myKey"));
      ...
      visualisation.push(`
        <div class="film-info">
          ...
          <div class="film-rating">
            <i class="fa fa-star rating-item ${localObj.nameAndStar[i][7]}" data-rate="5" data-film="${localObj.nameAndStar[i][2]}" aria-hidden="true"></i>
            <i class="fa fa-star rating-item ${localObj.nameAndStar[i][6]}" data-rate="4" data-film="${localObj.nameAndStar[i][2]}" aria-hidden="true"></i>
            <i class="fa fa-star rating-item ${localObj.nameAndStar[i][5]}" data-rate="3" data-film="${localObj.nameAndStar[i][2]}" aria-hidden="true"></i>
            <i class="fa fa-star rating-item ${localObj.nameAndStar[i][4]}" data-rate="2" data-film="${localObj.nameAndStar[i][2]}" aria-hidden="true"></i>
            <i class="fa fa-star rating-item ${localObj.nameAndStar[i][3]}" data-rate="1" data-film="${localObj.nameAndStar[i][2]}" aria-hidden="true"></i>
          </div>
        </div>  
      `);
    }
  ...
  section.addEventListener('click', (e) => {
    if (!e.target.classList.contains('active')) {
      ...
      // function sorta(){
      //   for (let i = 1; i < filmInfo.nameAndStar.length; i++) {

      //     for (let j = 1; j < filmInfo.nameAndStar.length; j++) {
        
      //       if( filmInfo.nameAndStar[j][0] < filmInfo.nameAndStar[j-1][0] ){
      //         let change_obj = filmInfo.nameAndStar[i];
      //         filmInfo.nameAndStar[i] = filmInfo.nameAndStar[i-1];
      //         filmInfo.nameAndStar[i-1] = change_obj;
      //       }
      //     }
      //   }
      // }
